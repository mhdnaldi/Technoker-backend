...
const fs = require("fs")
const { postPortofolio, getPortofolioById, deletePortofolioById } = require("../model/m_portofolio");
module.exports = {
    ...
    deletePortofolio: async (request, response) => {
        ...
        try {
            ...
            if (checkId.length > 0) {
                ...
                return helper.response(response, 404, `Prtofolio with ID ${id} is not found!`);
            }
        } catch (e) {
            return helper.response(response, 400, "Bad Request", e);
        }
