...
const { checkRecruiter, postRecruiter } = require('../model/m_recruiter')
    ...
    loginrecruiter: async (request, response) => {
        ...
        try {
            ...
            if (checkData.length > 0) {
                ...
                if (checkPassword) {
                    const { recruiter_name, recruiter_email, recruiter_company, recruiter_position, recruiter_phone, recruiter_profile_image, recruiter_background_image, recruiter_location, recruiter_about, recruiter_instagram, recruiter_linkedin, role } = checkData[0]
